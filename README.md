...
The original credential and token JSON files used during development are not included in the repository because they are tied to the developer’s personal Google account, authentication sessions, and Google Cloud configuration. For security and privacy reasons, new OAuth credentials and tokens are advised to be generated by the user during setup.
